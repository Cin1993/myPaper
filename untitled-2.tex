...
\begin{table}[h] 
	...
	\resizebox{\textwidth}{12mm}
	{
		...
		% \toprule[1.5pt]
		\toprule
		\textbf{Scheme} & \textbf{Access Structure} & \textbf{Hidden Policy} & \textbf{Testing} & \textbf{Outsourcing} & \textbf{Verification} &\textbf{Security Model} \\  \hline%用&来分隔单元格的内容 \\表示进入下一行
		\midrule %画一个横线，下面的就都是一样了，这里一共有4行内容
		Nishide & AND-gates & \checkmark & $\times$   & $\times$   & $\times$   & selectively-CCA \\
		Lai     & AND-gates & \checkmark & $\times$   & $\times$   & $\times$   & adaptively-CCA  \\
		Luan    & Tree 		& $\times$   & $\times$   & $\times$   & $\times$   & selectively-CPA \\
		Li		& AND-gates & \checkmark & \checkmark & $\times$   & $\times$   & selectively-CPA \\
		Xu		& Tree		& \checkmark & $\times$   & $\times$   & $\times$   & selectively-CPA \\	
		Ours    & Tree 		& \checkmark & \checkmark & \checkmark & \checkmark & adaptively-CCA  \\
		\bottomrule
		\end{tabular}
	}
	\label{propertyTable}
...
\begin{table}[h]
	...
	\resizebox{\textwidth}{10mm}
	{
		\begin{tabular}{cccc}
			\toprule
			\textbf{Scheme} & \textbf{Public Key} & \textbf{Secret Key} & \textbf{Ciphertext} \\ \hline
			\midrule
...
% 	{
% 		\begin{tabular}{cccc}
% 			\textbf{Scheme} & \textbf{Encryption Time} & \textbf{Decryption Time} & \textbf{} &
