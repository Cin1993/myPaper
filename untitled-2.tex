...
\titlerunning{A Verifiable Hidden Policy CP-ABE with Decryption Testing Scheme}        % if too long for running head
...
\begin{abstract}
	As a new kind of cryptographic primitive, attribute-based encryption(ABE) is widely used in various complex scenarios since it has the characteristics of access control while encrypting messages.
...
\section{Introduction}
	...
	However, the premise of all is that the cloud computing platform can provide a secure, trusted environment.
	...
	In VANET system, OBU and RSU are mostly resource-limited embedded devices with tiny size, which have poor performance compared to desktops or laptops in term of computing.
	...
	\begin{figure}
		...
		\caption{Typical VANET Structure}
		\label{VANET}
	\end{figure}
...
% a new paragraph
Table\ref{propertyTable} indicates the property comparisons between our scheme and other related schemes. 
...
\begin{table}[htbp] 
	...
	\resizebox{\textwidth}{13mm}
	{
		...
		\hline 
		Nishide\cite{nishide2008attribute} & AND-gate  & \checkmark & $\times$   & $\times$   & $\times$    \\ \hline
		Lai\cite{lai2011fully}     & AND-gate  & \checkmark & $\times$   & $\times$   & $\times$    \\ \hline
		Luan\cite{ibraimi2009efficient}    & Tree 		& $\times$   & $\times$   & $\times$   & $\times$    \\ \hline
		Li\cite{li2016ciphertext}		& AND-gate  & \checkmark & \checkmark & $\times$   & $\times$    \\ \hline
		Xu\cite{xu2013tree}		& Tree		& \checkmark & $\times$   & $\times$   & $\times$    \\ \hline	
		Hur\cite{hur2013attribute}		& Tree		& \checkmark & $\times$   & \checkmark & $\times$    \\ \hline
...
\begin{table}[htb]
	...
	\resizebox{\textwidth}{9mm}
	{
		\begin{tabular}{|c|c|c|c|}
			...
			% Nishide & a & a & a  \\ \hline
			Hur\cite{hur2013attribute} & $2|\mathbb{G}|+|\mathbb{G}_T|$ & $(3k+1)|\mathbb{G}|$ & $(2t+1)|\mathbb{G}|+2\mathbb{G}_T$  \\ \hline
			Helil\cite{helil2017cp} & $3|\mathbb{G}|+ |\mathbb{G}_T|$ & $(3k+1)|\mathbb{G}|$ & $(2t+1)|\mathbb{G}|+2\mathbb{G}_T$  \\ \hline
			
			Muller\cite{muller2011hiding} & $2|\mathbb{G}_T|+(N+2)|\mathbb{G}|$ & $(2n+2)|\mathbb{G}|$ & $(N+n)|\mathbb{G}|+2|\mathbb{G}_T|$ \\ \hline   
...
\begin{table}[htb]
	...
	\resizebox{\textwidth}{9mm}
	{
		\begin{tabular}{|c|c|c|c|c|}
			\hline  
			\multirow{3}{*}{\textbf{Scheme}} & \multirow{3}{*}{\textbf{Encryption}} & \multicolumn{3}{c|}{\textbf{Decryption}} \\			
		\cline{3-5}          &       & \multicolumn{2}{c|}{\textbf{Transform$_{out}$}} & \multirow{2}{*}{\textbf{LocalDec}} \\
		\cline{3-4}          &       & \textbf{Testing} & \textbf{PreDec} &  \\
		\hline
		...
		% Xu      &  &  &  & \\ \hline
		Hur\cite{hur2013attribute}     &$(3t+3)\mathbb{G}+(t+1)P$     & $-$     & $(2|R|+1)P+\mathbb{G}_T$     & $P+3\mathbb{G}_T$ \\ \hline
		Helil\cite{helil2017cp}     &$(3t+4)\mathbb{G}+(t+2)P$     & $-$     & $(2|R|+1)P+\mathbb{G}_T$     & $P+3\mathbb{G}_T$ \\ \hline
		Muller\cite{muller2011hiding}  &  $(N+n)\mathbb{G} + 2\mathbb{G}_T$ & $-$ & $-$ & $(3n+2)P+(3n+2)\mathbb{G}_T$ \\ \hline
...
\begin{figure}[htb]
	...
	\caption{(a)Comparison of the encryption time. (b)Comparison of the decryption time.}
...
\begin{figure}[htb]
	...
	\caption{Storage cost of private key}
	\label{fig.privateKeySize}
\end{figure}

\section{Conclusions}
In this paper, we present a new CP-ABE scheme called VHPDT.
Meanwhile, based on it, we construct a efficient privacy-presenting message transmission (EPPMT) system for VANET.
It adopts the techniques of outsourcing and decryption testing, which is suitable for VANET mainly involving embedded devices.
In addition, EPPMT system is able to fully protect user privacy, and only users who meet a specific access structure can read the content of the ciphertext.
Finally we prove the security of our scheme and do plentiful simulation experiments to illustrate the advantages of the scheme.

However, there are still some open issues worth studying. 
In our scheme, the size of ciphertext is related to the number of \textit{LPNodes} and attributes.
In case of complex practical application scenarios, the ciphertext length is too long.
Therefore, designing a fixed ciphertext length scheme would be a promising direction. 
