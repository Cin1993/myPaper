...
\title{A Hidden Tree Structure CP-ABE with Decryption Testing Scheme and its Application in Cloud Computing}
...
\keywords{ciphertext policy attribute-based encryption;hidden policy;cloud computing;}
\end{abstract}

\section{Introduction}
	In the era of data explosion, the development of cloud computing has given users a better and more convenient way to share data and store data.
	...
	However, since the user stores the data on the cloud platform and the data is out of the user's control, it will bring out an apparent issue that if malicious users attack cloud server and obtain the data belonging to normal users, the user's privacy will be leaked and even causes incalculable losses. 
	Therefore, how to ensure the privacy of users and the security of cloud data has become an urgent problem to be solved. 
	ABE as a novel encryption notation, not only provides access control but also provides the function of encrypting data, consequently, it is regarded as a positive method to handle this matter. 
	However, although attribute-based encryption can solve above problems, it has another hidden danger which is access structure is passed in clear text during the process of encryption and decryption.
	Once some malicious attackers steal the access structure stored in clear text, the user's privacy will be revealed.
...
At last, outsourced server sets $T_1 = \widehat{C}$ and transfers $CT'=\left( T_1, X \right)$ to DataUser.
	...
	\item Verify(CT,CT'): DataUser verifies the correctness of result generated from outsourced server.
		...
		\begin{itemize}
			\item[a).] If $T_1=\widehat{C}$ holds, let $\textbf{DF}=X^{\frac{1}{z}}$
		\end{itemize}
\end{itemize}
